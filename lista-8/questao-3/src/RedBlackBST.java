...
public class RedBlackBST<Key extends Comparable<Key>, Value> {
    ...
    //complexidade O(log n), pois a arvore é balanceada
    private int alturaNegraRec(Node x) {

        if (x == null)
            return -1;

        int altEsq = alturaNegraRec(x.left), altDir = alturaNegraRec(x.right);

        if (altEsq != altDir) { //fere os requisitos de RBtree
            throw new RuntimeException("Arvore invalida.");
        }

        return altEsq + (x.color == BLACK ? 1 : 0);
    }
}
